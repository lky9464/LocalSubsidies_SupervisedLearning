...
from src.scoring.score_distribution import score_bin_labels
...
@router.get("/api/runs/{run_id}/models/score-distribution")
def models_score_distribution(
    run_id: str,
    cfg=Depends(get_cfg),
    repo=Depends(get_repo),
) -> dict:
    ranking = repo.get_ranking(run_id)
    labels_map = build_algo_labels_map(cfg)
    panels = build_score_distribution_panels(
        cfg, ranking, run_id=run_id, labels_map=labels_map
    )
    return {
        "run_id": run_id,
