...
## 5. 타겟 포착 분포 · 점검 우선순위표 (주 모델 = RandomForest)
...
Stacked는 **분쟁·재검토 시 참고**로만 사용 (단독 컷오프·선정 기준 비포함).
...
2. 로컬 실행:
   ...
   python scripts/11_score_inference.py --algo stacked_ensemble
   ```
...
4. **업무 배포·점검 우선순위표는 주모델(RF) 상위% 구간 우선** + 보조(CatBoost)는 동등급 내 정렬  
